...
function Actor() constructor {
	...
	static Activate = function( _effect, _effectType ) {
		...
		while( ds_list_size( global.effectChainRing ) > 0 ) {
			...
			// Se qualcuno vuole aggiungersi alla chain, mette tutto in global.effectChainRing
			...
			for(var i=0; i<ds_list_size(chain); i++ ) {
				...
				for(var j=ds_list_size(global.effectListeners)-1;j>=0;j--)
					global.effectListeners[|j]( effect[1], effect[2], i ) // listener( effectType, actor, effectIndex )
			}
		}
...
function Deck() {
	// Il mazzo è una lista, dove l'indice 0 è la carta in cima, quella che si pescherà
	cards = ds_list_create()
	static Add = function(_card) {
		ds_list_add(cards,_card)
	}
